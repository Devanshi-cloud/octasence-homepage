...
const DropdownMenuContent: React.FC<{
  ...
  <div
    className={`bg-slate-900 border border-white/10 shadow-2xl md:w-[400px] lg:w-[600px] text-sm rounded-xl p-6 ${className}`}
  >
    <div className="text-slate-400 mb-6 font-bold uppercase tracking-wider text-xs">{title}</div>
                ...
                <li key={item.href}>
                  {isExternal ? (
                    <a
                      ...
                      className="block p-3 rounded-xl hover:bg-cyan-500/10 hover:text-cyan-400 transition-all border border-transparent hover:border-cyan-500/20"
                    >
...
const Navbar: React.FC = () => {
  ...
  return (
    <div
      className={cn(
        "w-full fixed top-0 z-[10000] transition-all duration-300 translate-y-0",
        lastScrollY > 50 ? "bg-[#020617]/80 backdrop-blur-md border-b border-white/5 shadow-xl" : "bg-transparent"
      )}
    >
      <nav className="w-full">
        ...
        >
          ...
          <Link href="/" passHref className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-blue-900 flex items-center justify-center shadow-[0_0_15px_rgba(30,58,138,0.5)] border border-white/10 transition-all hover:scale-105 active:scale-95">
              <span className="text-white font-bold text-xl">O</span>
            </div>
            <span className={cn(
              "text-3xl font-bold transition-colors tracking-tighter text-white"
            )}>
              OctaSence
            </span>
          </Link>
          ...
          <button
            onClick={toggleMenu}
            className={cn(
              "focus:outline-none md:hidden transition-colors text-white"
            )}
          >
            ...
            {Object.entries(menuItems).map(([title, items]) => (
              <div key={title} className="relative group">
                <Link 
                  ...
                  className={cn(
                    "font-medium transition-colors text-base flex items-center text-white/80 hover:text-cyan-400"
                  )}
                >
                  {title}
                  <TbChevronDown className="ml-1 h-5 w-5 opacity-50" />
                </Link>
                <DropdownMenuContent
                  title={title}
                  items={items}
