...
const ServiceBackDecor = ({ type }: { type: number }) => {
  ...
  if (type === 0) {
    return (
      <div className="absolute inset-0 opacity-20 -z-10 pointer-events-none flex items-center justify-center">
        <svg width="400" height="400" viewBox="0 0 200 200" className="text-cyan-400">
          <circle cx="100" cy="160" r="50" fill="none" stroke="currentColor" strokeWidth="2" opacity="0.3" />
          {[...Array(3)].map((_, i) => (
            <motion.path
              key={i}
              ...
              animate={{ 
                d: [`M 100 0 Q ${50 + i * 50} 80 100 110`, `M 100 0 Q ${150 - i * 50} 80 100 110`, `M 100 0 Q ${50 + i * 50} 80 100 110`],
                opacity: [0.3, 0.8, 0.3]
              }}
  ...
  if (type === 1) {
    return (
      <div className="absolute inset-0 opacity-25 flex items-center justify-center -z-10 pointer-events-none">
        ...
        >
          <svg width="200" height="200" viewBox="0 0 24 24" className="text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.8)]">
            <path fill="currentColor" d="M12 2L4.5 20.29l.71.71L12 18l6.79 3 .71-.71L12 2z" />
            <motion.circle 
              cx="12" cy="12" r="2" fill="cyan"
      ...
      <div className="absolute inset-0 opacity-20 flex items-end justify-between -z-10 pointer-events-none px-4">
        {[...Array(8)].map((_, i) => (
          <motion.div
            ...
            initial={{ height: 0 }}
            animate={{ height: [`${10 + i * 8}%`, `${30 + i * 6}%`, `${10 + i * 8}%`] }}
    ...
    return (
      <div className="absolute inset-0 opacity-20 flex flex-col items-center justify-center -z-10 pointer-events-none gap-4">
        <div className="flex gap-1 h-32 items-end">
          {[...Array(5)].map((_, i) => (
            <motion.div key={i} className="w-4 bg-white" animate={{ height: [20, 100, 20] }} transition={{ duration: 2, repeat: Infinity, delay: i * 0.3 }} />
          ))}
        </div>
        <motion.div 
  ...
  return (
    <div className="absolute inset-0 opacity-20 flex items-center justify-center -z-10 pointer-events-none">
      <svg width="200" height="200" viewBox="0 0 24 24" className="text-cyan-400">
        <motion.path 
...
const IndustryCard = ({ name, bullets }: { name: string, bullets: string[] }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-black text-white mb-8 text-center border-b border-white/10 pb-4 capitalize">
        {name}
      </h3>
...
const ServiceFlipCard = ({ title, videoSrc, description, index }: { title: string, videoSrc: string, description: string[], index: number }) => {
      ...
      >
        <div className="absolute inset-0 w-full h-full backface-hidden bg-white/10 backdrop-blur-md rounded-[40px] overflow-hidden border-2 border-white/20 shadow-2xl flex flex-col">
          ...
          <div className="p-8 flex-1 flex items-center justify-center">
            <h3 className="text-2xl font-black text-white text-center leading-tight tracking-tight capitalize">
              {title}
            </h3>
          </div>
        </div>
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden bg-white/20 backdrop-blur-xl rounded-[40px] p-8 flex flex-col justify-center border-2 border-white/30 shadow-inner rotate-y-180 overflow-hidden">
          ...
          <h4 className="text-xl font-black text-white mb-6 border-b border-white/10 pb-2 text-center capitalize relative z-10">Capabilities</h4>
          <ul className="space-y-3 overflow-y-auto max-h-[300px] pr-2 custom-scrollbar relative z-10">
            {description.map((point, i) => (
              <li key={i} className="flex items-start gap-3">
                ...
                <span className="text-sm text-white font-bold leading-snug drop-shadow-md">{point}</span>
              </li>
            ))}
          </ul>
        </div>
...
export default function InfrastructureIntelligence() {
  return (
    <div className="min-h-screen bg-[#020617] tech-grid selection:bg-cyan-500 selection:text-white overflow-x-hidden">
      <Navbar />
      ...
      <main className="relative">
        ...
        <section className="relative pt-40 pb-24 md:pt-56 md:pb-40 bg-transparent text-center font-outfit overflow-hidden">
          ...
          <div className="container mx-auto px-6 max-w-6xl relative z-10">
            ...
            >
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white leading-[1.2] mb-10 tracking-tight">
                Mission-critical infrastructure <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-500 font-bold mt-4 block">
                  Intelligence as a service (IIaaS)
                </span>
              </h1>
              <p className="text-lg md:text-2xl text-slate-400 max-w-5xl mx-auto font-medium leading-relaxed mb-16">
              ...
              <motion.div
                animate={{ 
                  boxShadow: ["0 0 20px rgba(34,211,238,0.2)", "0 0 35px rgba(34,211,238,0.5)", "0 0 20px rgba(34,211,238,0.2)"]
                }}
                transition={{ duration: 2, repeat: Infinity }}
              ...
              >
                <Link
                  ...
                  className="inline-block px-14 py-5 bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-black rounded-xl hover:from-cyan-500 hover:to-blue-500 active:scale-95 transition-all border border-cyan-400/30 tracking-wider uppercase text-sm"
                >
                  Request Audit
                </Link>
              </motion.div>
        ...
        <section className="py-32 bg-transparent font-outfit">
          <div className="container mx-auto px-6">
            <div className="text-center mb-24">
              <h2 className="text-4xl md:text-6xl font-black text-blue-500 mb-6 tracking-tight capitalize [text-shadow:0_0_20px_rgba(59,130,246,0.3)]">Our solutions</h2>
        ...
        <section className="py-32 bg-transparent relative overflow-hidden font-outfit">
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="text-center mb-24">
              <h2 className="text-4xl md:text-6xl font-black text-blue-500 mb-6 tracking-tight capitalize [text-shadow:0_0_20px_rgba(59,130,246,0.3)]">Industries we serve</h2>
              <div className="w-24 h-2.5 bg-blue-500/40 mx-auto rounded-full" />
            </div>
