...
const StrobeCanvas: React.FC<{ containerRef: React.RefObject<HTMLDivElement> }> = ({ containerRef }) => {
  ...
  <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
    {Array.from({ length: 8 }, (_, i) => (
      <motion.div
        ...
        transition={{ duration: 5 + (i % 3) * 2, repeat: Infinity, ease: 'linear', delay: i * 1.8 }}
      />
    ))}
  </div>
);
...
const FlowingWave: React.FC = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20 -z-10">
    <svg className="w-full h-full">
      {[
        { d1: 'M-100 100 Q 50 150 200 100 T 500 150 T 800 100', d2: 'M-100 130 Q 50 80 200 130 T 500 80 T 800 130', stroke: '#4f7fff', sw: '2', dur: 10 },
        { d1: 'M-50 150 Q 80 200 250 150 T 550 200 T 850 150', d2: 'M-50 180 Q 80 130 250 180 T 550 130 T 850 180', stroke: '#7b9fff', sw: '1.5', dur: 14 },
        { d1: 'M-150 120 Q 30 170 180 120 T 480 170 T 780 120', d2: 'M-150 150 Q 30 100 180 150 T 480 100 T 780 150', stroke: '#4f7fff', sw: '1', dur: 18 },
      ].map((w, i) => (
        <motion.path
          key={i}
          animate={{ d: [w.d1, w.d2, w.d1] }}
          transition={{ duration: w.dur, repeat: Infinity, ease: 'linear', delay: i }}
          stroke={w.stroke}
          strokeWidth={w.sw}
          fill="transparent"
          opacity={0.6 - i * 0.15}
          ...
          <div className="container mx-auto px-6 max-w-5xl relative z-10">
            ...
            >
              ...
              <div className="flex items-center justify-center gap-4 flex-wrap">
                <motion.div
                  animate={{ boxShadow: ['0 0 20px rgba(79,127,255,0.2)', '0 0 35px rgba(79,127,255,0.5)', '0 0 20px rgba(79,127,255,0.2)'] }}
                  transition={{ duration: 2, repeat: Infinity }}
              ...
              >
                {[
                  ...
                  <React.Fragment key={i}>
                    {i > 0 && <div className="w-px h-9 bg-white/13 hidden sm:block" />}
